...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate horror music stems using Meta AudioCraft MusicGen."
    )
    parser.add_argument(
        "--prompt",
        required=True,
        help="Generation prompt text focused on horror atmosphere.",
    )
    ...
    parser.add_argument(
        "--output-dir",
        default=str(script_root / "generated"),
        help="Output directory.",
    )
    parser.add_argument("--sample-rate", type=int, default=48000, help="Target sample rate.")
    parser.add_argument("--model", default="facebook/musicgen-medium", help="MusicGen model.")
...
def main() -> None:
    ...
    for idx in range(args.count):
        ...
        audio, sample_rate = generator.generate_music(request)
        audio = peak_normalize(audio, target_dbfs=-1.0)
